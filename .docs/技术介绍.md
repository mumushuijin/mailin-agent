...
| 路由 | Vue Router | ^5.0 |
| 状态 | Pinia（脚手架保留，业务状态以组件内 ref 为主） | ^3.0 |
...
超大工具结果落盘到 `workspace/tool_results/`，上下文中仅保留引用，避免撑爆 Token 预算。
...
位于 `workspace/bootstraps/`，定义 Agent 身份与行为：

| 文件 | 用途 |
|------|------|
| `IDENTITY.md` | Agent 名称、角色定位 |
| `SOUL.md` | 人格与语气风格 |
...
| 包名 | config_key | 工具 | 说明 |
|------|------------|------|------|
| filesystem | `filesystem` | read/write/replace/list/search/glob/mkdir/move/delete | 工作区沙箱内文件操作 |
| shell | `shell` | `run_shell` | 命令行执行，含安全守卫与审批 |
| memory | `memory` | list/get/grep/add/consolidate | 记忆读写与沉淀 |
| calculator | `calculator` | `python_calculator` | AST 安全 Python 求值 |
| datetime | `datetime` | `get_current_time` | 当前时间（IANA 时区） |
| web | `web_search` | `web_search` | Tavily 搜索（未配置时回退 DuckDuckGo） |
| mcp | `mcp` | 动态 MCP 工具 + `mcp_status` | 外部 MCP 服务器集成 |

### 8.3 Shell 工具安全机制

- `safety_mode`：启用命令守卫
- `auto_approve_patterns`：白名单命令免审批（如 `git status`、`ls`）
- 高风险命令触发 **interrupt**，前端弹出审批 Modal
- 进程注册表跟踪，支持取消
...
| 阶段 | 工具 | 说明 |
|------|------|------|
| 热工具 | 直接 `bind_tools` | 默认：read_file、run_shell、memory_* 等 |
| 冷工具 | `tool_search` → `tool_describe` → `tool_call` | BM25 检索后按需调用 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/chat/send` | 同步对话（JSONResponse） |
| POST | `/api/chat/send/sync` | 同步对话（强类型 ChatResponse） |
...
| `/sessions` | SessionsView | 会话列表 CRUD |
| `/memory` | MemoryView | 每日工作记忆浏览 |
| `/config` | ConfigView | Agent 配置编辑与重置 |
...
| 套件 | 路径 | 说明 |
|------|------|------|
| smoke | `eval/datasets/smoke/` | 冒烟测试 |
| retrieval | `eval/datasets/retrieval/` | 记忆检索评测（RAGAS 格式） |
